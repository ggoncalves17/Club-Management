...
def utilizadores(request):
    ...
    # LISTA DE TODOS OS UTILIZADORES - GET
    ...
    # INSERÇÃO DE NOVO UTILIZADOR - POST
...
def modalidades(request):
    ...
    if request.method == 'GET':
        ...
        modalidades = Modalidade.objects.filter(clube_id=id_clube).order_by('nome')
...
@permission_classes([IsAuthenticated])
def utilizador(request, id):

    # INFO DE UTILIZADOR - GET
    if request.method == 'GET':
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def elementos_disponiveis(request, id):
...
@permission_classes([IsAuthenticated])
def inscricoes_jogador(request, id):
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def historico_categorias(request):
...
def categoria(request, id):
    ...
    if Categoria.objects.filter(nome__iexact=nome, clube=id_clube).exclude(id=id).exists():
        return Response({"mensagem": "Já existe uma categoria com o mesmo nome."}, status=404)
    else:
        try:
            ultimo_registo = Historico_Categoria.objects.filter(categoria=id, data_final__isnull=True).order_by('-data_inicial').first()
...
@api_view(['PUT'])
@permission_classes([IsAuthenticated])
def estado_categoria(request, id):
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def utilizadores_disponiveis(request):
...
@api_view(['PUT'])
@permission_classes([IsAuthenticated])
def regista_pagamento_quotas(request, id):
